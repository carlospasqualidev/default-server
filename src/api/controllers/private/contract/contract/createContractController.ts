...
import { createContractService } from '../../../../services/contract/contract';
...
import {
  createManyContractInstallmentService,
  generateContractInstallmentCompoundInterestService,
  generateContractInstallmentFixedInterestService,
} from '../../../../services/contract/contractInstallments';
...
export async function createContractController(req: Request, res: Response) {
  ...
  if (gracePeriodPaymentMonths >= paymentTermMonths) {
    throw new ErrorMessage({
      statusCode: '422 UNPROCESSABLE CONTENT',
      message: 'O período de carência não pode ser maior ou igual ao prazo de pagamento.',
    });
  }
  ...
  if (type === 'fixedInterest') {
    installments = generateContractInstallmentFixedInterestService({
  ...
  if (type === 'compoundInterest') {
    installments = generateContractInstallmentCompoundInterestService({
      contractId: contract.id,
      paymentStartDate,
      totalValue,
      paymentTermMonths,
  ...
  await createManyContractInstallmentService({
    data: installments,
  });
